...
public final class ReciprocalArraySum {
	...
	protected static double parManyTaskArraySum(final double[] input, final int ntasks) {
		System.setProperty("java.util.concurrent.ForkJoinPool.common.parallelism", "8");
		return ReciprocalArraySumTask.parManyTaskArraySum(input, ntasks);

	}

	protected static double parArraySum(final double[] input) {
		System.setProperty("java.util.concurrent.ForkJoinPool.common.parallelism", "8");
		return ReciprocalArraySumTask.parArraySum(input);
	}
	...
	private static class ReciprocalArraySumTask extends RecursiveAction {
		 ...
		 * seqArraySum, but use two tasks running in parallel under the Java
		 * Fork Join framework. You may assume that the length of the input
		 ...
		 * above utilities getChunkStartInclusive and getChunkEndExclusive
		 * helpful in computing the range of element indices that belong to each
		 * chunk.
		 *
		 * @param input
		 *            Input array
		...
		protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
			// this fixes the limit on 2 processes
